...
const initialUsersCatalog = [
  ...
  { id: 'USR-007', name: 'Mario Sandoval', initials: 'MS', role: 'Administrador', area: 'Administracion APEX', active: true, primary: true },
]
...
function isProformaDetailComplete(proforma) {
  const lines = proforma?.lines || []
  return lines.length > 0 && lines.every((line) => (
    String(line.item || '').trim()
    && String(line.sku || '').trim()
    && String(line.description || '').trim()
    && numericValue(line.required) > 0
    && Number.isFinite(Number(line.available))
    && String(line.authorization || '').trim()
  ))
}
...
function selectedPELinesHaveEstimatedDate(linesList) {
...
function canAddFiniquitoDocument(role, currentCase, proformasList = []) {
  ...
  return (role === 'Facturacion' || role === 'Administrador') && currentCase.otExecutionApproved && currentCase.state !== 'Cerrado' && hasAvailabilityWindow
}
...
function downloadHrefFor(document) {
  ...
  const content = [
    ...
    document?.preview || 'Documento demo del expediente APEX.',
  ].join('\n')
...
function App() {
  ...
  function handleModalConfirm(name, values) {
    ...
    if (name === 'Cambiar fecha') {
      if (!hasPEOC(currentPE)) {
        showToast('Primero registre la OC JDE del Pedido Especial')
        return
      }
    ...
    if (name === 'Confirmar disponibilidad fisica y cantidades') {
      ...
      if (!hasPEOC(currentPE) || !selectedPELinesHaveEstimatedDate(peLinesData)) {
        showToast('La recepcion requiere OC JDE y fecha estimada en las lineas seleccionadas')
        return
      }
  ...
  function handleConfirmAvailability() {
    ...
    if (!isProformaDetailComplete(targetProforma)) {
      showToast('Complete todos los datos del detalle antes de confirmar')
      return
    }
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <nav className="nav">
          {role !== 'Compras' && <NavButton active={section === 'cases'} icon={BriefcaseBusiness} label="Casos" onClick={() => resetToList('cases')} />}
...
function ExpedienteTab({ role, activeUser, currentCase, proformasData, onAddAdditionalDocument, onAddHiddenDamage, onAddFiniquito, onLoadDocument, onUpdateDocumentMeta, onRemoveDocument, onMarkExpedienteComplete, onMarkHiddenDamageComplete }) {
  ...
  const canMarkComplete = canManageDocuments && requiredDocumentsReady && currentCase.state === 'Expediente pendiente de completar'
...
function ProformasTab({ role, proformasData, selectedProformaId, currentCase, onSelectProforma, onRenameProforma, onLoadExcelDetail, onClearProformaDetail, onRequestAvailabilityValidation, onProformaLineChange, onPrepareSpecialOrder, onModal, onDrawer }) {
  ...
  const canConfirmAvailability = selectedProformaCanAdvance && (role === 'Administrador' || role === 'Bodega de Enderezado') && currentCase.state === 'En validación de disponibilidad' && !selectedProforma?.availabilityConfirmed && isProformaDetailComplete(selectedProforma)
      ...
      <DataTable
        ...
        rows={ordersData.map((item) => [
          ...
          <div key="actions" className="row-actions">
            <button type="button" className="row-action" onClick={() => onOpenPE(item)}>Abrir PE</button>
            {canRegisterJdeForPE(role, item) && <button type="button" className="row-action" onClick={() => onRegisterJDE(item)}>JDE</button>}
            {role !== 'Compras' && <button type="button" className="row-action" onClick={() => onOpenCase(cases[0])}>Caso</button>}
          </div>,
        ])}
      />
    </section>
...
function PEWorkspace({ role, currentPE, peLinesData, tab, setTab, onModal, onOpenCase, onToggleLine, onToggleAllLines, onUpdateLine, onRevertReception, canRevertReception }) {
  ...
  const selectedLinesHaveDate = selectedPELinesHaveEstimatedDate(normalizedLines)
  const canRegisterOC = (role === 'Compras' || role === 'Administrador') && !peComplete && currentPE.state === 'PE solicitado en JDE' && !hasOc
  const canAssignDate = (role === 'Compras' || role === 'Administrador') && !peComplete && hasOc && hasSelectedLines
  const canShowConfirmPhysical = canManagePEReceipt(role) && !peComplete && hasOc && hasSelectedLines
  const canConfirmPhysical = canShowConfirmPhysical && selectedLinesHaveDate && selectedPELinesReadyForReceipt(normalizedLines)
  const confirmPhysicalTitle = canConfirmPhysical ? 'Confirmar disponibilidad fisica' : 'Todas las filas seleccionadas deben tener fecha estimada, Recibida > 0 y mayor a lo ya confirmado'
  return (
    <section className="workspace">
      <WorkspaceHeader
        ...
        actions={(
          <>
            ...
            {role !== 'Compras' && <button type="button" className="secondary" onClick={onOpenCase}><BriefcaseBusiness size={16} /> Ver caso</button>}
...
function PELinesTab({ role, currentPE, peLinesData, onToggleLine, onToggleAllLines, onUpdateLine }) {
  ...
  const canUseSelection = canManagePEReceipt(role) && hasOc
  const canEditReceived = canUseSelection && hasOc && !readOnly
  const canEditLineOC = (role === 'Compras' || role === 'Administrador') && hasOc && !readOnly
        ...
        rows={peLinesData.map((line, index) => ({
          ...
          cells: [
            ...
            <input
              ...
              disabled={!canEditReceived || !line.selected || !parseDateValue(line.estimatedDate)}
...
function Modal({ name, role, currentCase, currentProforma, currentPE, peLinesData, lastReceptionSnapshot, dateChangeReasons, onConfirm, onClose }) {
  ...
  const canConfirmAvailability = !isConfirmAvailability || (hiddenDamageCanAdvanceInventory(currentCase, currentProforma) && isProformaDetailComplete(currentProforma))
  const canConfirmPhysical = !isConfirmPhysical || (selectedPELinesHaveEstimatedDate(peLinesData) && selectedPELinesReadyForReceipt(peLinesData))
  ...
  const confirmLabel = name === 'Marcar expediente como completo' ? 'Confirmar envio a Bodega' : name === 'Cerrar caso' ? 'Cerrar caso' : name === 'Proceder con OT' ? 'Proceder con OT' : isRegisterJDE ? (registeringJde ? 'Confirmar solicitud JDE' : 'Registrar OC JDE') : isGeneratePE ? 'Generar Pedido Especial' : isConfirmAvailability ? 'Confirmar Disponibilidad' : name === 'Cambiar fecha' ? 'Asignar fecha' : name === 'Confirmar disponibilidad fisica y cantidades' ? 'Confirmar disponibilidad fisica' : name === 'Revertir ultima confirmacion' ? 'Revertir confirmación' : isCreateCase ? 'Crear caso demo' : isAddDocument ? 'Agregar documento' : isFiniquito ? 'Aplicar finiquito' : isHiddenDamage ? 'Agregar daño oculto' : 'Guardar'
